...
class fudan:
	...
	def start(self):
		...
		if content["lecture"]:
			cur.execute('drop table if exists fudan_lecture')
			cur.execute('create table if not exists fudan_lecture(id int(11) primary key auto_increment,title varchar(255),intro text,guest text,video_info text,addr text,url varchar(255))')
			sql = 'insert into fudan_lecture(title,intro,guest,video_info,addr,url) values(%s,%s,%s,%s,%s,%s)'
